...
let tickets = [];       // 発行済み整理券
let checkinList = [];   // チェックイン済み
let checkoutList = [];  // チェックアウト済み
let currentNumber = 0;  // 呼び出し番号
let maxInside = 5;      // ADMIN/adminで設定可能
...
function updateCallNumber(){
  ...
  currentNumber = exited + Math.min(maxInside, tickets.length - exited - inside);
}
...
app.use("/admin/admin",(req,res,next)=>{
  ...
  if(!adminPassword){
    if(pw){
      ...
      return res.send("管理者パスワードをURLパラメータで設定してください。例: /admin/admin?pw=XXXX");
    }
  } else {
